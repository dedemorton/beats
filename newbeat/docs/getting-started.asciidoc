...
//* <<{beatname_lc}-modules-quickstart>>
//* <<setup-repositories>>
...
example: +PowerShell.exe -ExecutionPolicy UnRestricted -File .{backslash}install-service-{beatname_lc}.ps1+.
endif::[]
...
//Use block delimiters (--) to keep text under list items indented.

To configure {beatname_uc}:

. ADD MAIN STEP FOR CONFIGURING THE INPUT.
+
--
["source","sh",subs="attributes"]
-------------------------------------------------------------------------------------
...
=== Step 3: Configure {beatname_uc} to use {ls}
...
["source","sh",subs="attributes"]
----------------------------------------------------------------------
sudo chown root {beatname_lc}.yml
sudo ./{beatname_lc} -e -c {beatname_lc}.yml -d "publish"
----------------------------------------------------------------------

// TODO: If this Beat requires root access, uncomment out the following lines and
// add a callout to the example.
...
+C:{backslash}ProgramData{backslash}{beatname_lc}{backslash}Logs+.
...
<<configuring-ingest-node,define ingest node pipelines>> or use {ls} to
parse the data into the fields expected by the dashboards. If you are using
{ls}, see the
{logstashdoc}/logstash-config-for-{beatname_lc}-modules.html[configuration examples]
in the {ls} documentation for help parsing the log formats supported
by the dashboards.

Here is an example of the {beatname_uc} system dashboard:

//[role="screenshot"]
//image:./images/add-name-here.png[]
