...
The `setup.dashboards` section of the +{beatname_lc}.yml+ config file contains options
for automatically loading the sample {beatname_uc} dashboards.
Dashboard
loading is disabled by default. To load the dashboards, you can either enable dashboard loading here in the configuration file, or you can run the `setup` command to load the dashboards.
...
following options: <<directory-option>>, <<url-option,directory,`url`>>, or <<file-option,`file`>>.
